...
router.post('/users', auth, roles(['admin']), async (req, res) => {
  try {
    ...
    await ActivityLog.logActivity(req.user._id, 'created', `user: ${user.name}`, 'user', { 
      userId: user._id,
      email: user.email,
      role: user.role 
    });
...
router.put('/users/:id', auth, roles(['admin']), async (req, res) => {
  try {
    ...
    // Log activity - ADD THIS
    await ActivityLog.logActivity(req.user._id, 'updated', `user: ${user.name}`, 'user', { 
      userId: user._id,
      email: user.email,
      role: user.role,
      fieldsUpdated: Object.keys(req.body).filter(key => key !== 'password')
    });
...
router.delete('/users/:id', auth, roles(['admin']), async (req, res) => {
  try {
    ...
    // Log activity before deletion - ADD THIS
    await ActivityLog.logActivity(req.user._id, 'deleted', `user: ${user.name}`, 'user', { 
      userId: user._id,
      email: user.email,
      role: user.role 
    });
...
router.delete('/tutorials/:id', auth, roles(['admin']), async (req, res) => {
  try {
    ...
    // Log activity - ADD THIS
    await ActivityLog.logActivity(req.user._id, 'deleted', `tutorial: ${tutorial.title}`, 'tutorial', { 
...
router.delete('/quizzes/:id', auth, roles(['admin']), async (req, res) => {
  try {
    ...
    // Log activity - ADD THIS
    await ActivityLog.logActivity(req.user._id, 'deleted', `quiz: ${quiz.title}`, 'quiz', { 
      quizId: quiz._id,
...
// ----------------------
// 🧩 Get Activity Log - ADD THIS NEW ENDPOINT
// ----------------------
router.get('/activity', auth, roles(['admin']), async (req, res) => {
  try {
    ...
    res.status(500).json({ success: false, msg: 'Server error fetching activity log' });
  }
});

// ----------------------
// 🧩 Export Data - ADD THIS NEW ENDPOINT
// ----------------------
router.get('/export/:type', auth, roles(['admin']), async (req, res) => {
  try {
    ...
    // Log export activity
    await ActivityLog.logActivity(req.user._id, 'exported', `${type} data`, 'system', { count: data.length });
    ...
    res.status(500).json({ success: false, msg: 'Server error exporting data' });
  }
});

// ----------------------
// 🧩 Import Data - ADD THIS NEW ENDPOINT
// ----------------------
router.post('/import/:type', auth, roles(['admin']), async (req, res) => {
  try {
    const { type } = req.params;
    const { data } = req.body;
    ...
    switch (type) {
      ...
      case 'tutorials':
        for (const item of data) {
          try {
            const { _id, ...tutorialData } = item;
            const tutorial = new Tutorial(tutorialData);
            await tutorial.save();
            importedCount++;
          } catch (error) {
            errors.push(`Tutorial ${item.title}: ${error.message}`);
          }
        }
        break;
      case 'quizzes':
        for (const item of data) {
          try {
            const { _id, ...quizData } = item;
            const quiz = new Quiz(quizData);
            await quiz.save();
            importedCount++;
          } catch (error) {
            errors.push(`Quiz ${item.title}: ${error.message}`);
          }
        }
        break;
      default:
        return res.status(400).json({ success: false, msg: 'Invalid import type' });
    }

    // Log import activity
    await ActivityLog.logActivity(req.user._id, 'imported', `${type} data`, 'system', { 
      importedCount, 
    ...
    res.json({
      success: true,
      msg: `Imported ${importedCount} ${type} successfully`,
      importedCount,
      errors: errors.length > 0 ? errors : undefined
    });
  } catch (err) {
    console.error('Import error:', err);
    res.status(500).json({ success: false, msg: 'Server error importing data' });
  }
});
