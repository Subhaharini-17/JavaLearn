...
export default function QuizzesManagement() {
  ...
  const fetchQuizzes = async () => {
    try {
      const token = localStorage.getItem("token");
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'https://javalearn-dac0.onrender.com/api'}/quizzes`, {
        headers: { 
  ...
  const fetchTopics = async () => {
    try {
      const token = localStorage.getItem("token");
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'https://javalearn-dac0.onrender.com/api'}/topics`, {
        headers: { 
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
        }
  ...
  const handleSubmitQuiz = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const url = editingQuiz 
        ? `${process.env.NEXT_PUBLIC_API_URL || 'https://javalearn-dac0.onrender.com/api'}/quizzes/${editingQuiz._id}`
        : `${process.env.NEXT_PUBLIC_API_URL || 'https://javalearn-dac0.onrender.com/api'}/quizzes`;
  ...
  const handleDeleteQuiz = async (quizId: string) => {
    ...
    try {
      const token = localStorage.getItem("token");
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'https://javalearn-dac0.onrender.com/api'}/admin/quizzes/${quizId}`, {
        method: 'DELETE',
        headers: { 
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
